...
// import { useGetRoomDetailClientQuery } from "@/app/component/admin/room/room";
...
//   );
//   const [mergedSeatMap, setMergedSeatMap] = useState<any[]>([]);
...
//   // 2️⃣ Lấy roomData dựa vào roomId từ showtime
//   const roomIdNum = dataSeatMap?.roomId;
//   const { data: roomData } = useGetRoomDetailClientQuery(
//     Number(roomIdNum || 0),
//   );

//   // 3️⃣ Merge seatLayout từ roomData vào seatMap showtime
//   useEffect(() => {
//     if (!dataSeatMap?.seatMap) return;
//     if (!roomData?.seatLayout) {
//       setMergedSeatMap(dataSeatMap.seatMap);
//       return;
//     }

//     let roomSeatMap: typeof dataSeatMap.seatMap = [];
//     try {
//       roomSeatMap = JSON.parse(roomData.seatLayout);
//     } catch (e) {
//       console.error("Failed to parse room.seatLayout", e);
//       setMergedSeatMap(dataSeatMap.seatMap);
//       return;
//     }

//     const merged = roomSeatMap.map((roomRow) => {
//       const showRow = dataSeatMap.seatMap.find(
//         (r) => r.rowLabel === roomRow.rowLabel,
//       );
//       if (!showRow) return roomRow;

//       const mergedSeats = roomRow.seats.map((roomSeat) => {
//         const showSeat = showRow.seats?.find(
//           (s) => s.code === roomSeat.code || s.number === roomSeat.number,
//         );
//         return {
//           ...roomSeat,
//           status: showSeat?.status ?? roomSeat.status,
//           price: showSeat?.price ?? roomSeat.price,
//         };
//       });

//       return { ...roomRow, seats: mergedSeats };
//     });

//     setMergedSeatMap(merged);
//   }, [dataSeatMap, roomData]);

//   // 4️⃣ Tạo map giá cho store
//   useEffect(() => {
//     if (!mergedSeatMap || mergedSeatMap.length === 0) return;
//     const map: Record<string, number> = {};
//     mergedSeatMap.forEach((row) => {
...
//   }, [mergedSeatMap, dispatch]);

//   // Toggle seat
//   const toggleSeat = (seatId: string) => {
//     if (!mergedSeatMap?.length) return;
//     const next = selectedSeats.includes(seatId)
//       ? selectedSeats.filter((s) => s !== seatId)
//       : [...selectedSeats, seatId];
//     const validation = validateSeatRules(mergedSeatMap, next);
//     if (!validation.valid) {
...
//     const validation = validateSeatRules(mergedSeatMap, selectedSeats);
//     if (!validation.valid) {
//       n.error(validation.message);
//       return;
//     }
...
//               {mergedSeatMap.map((row) => {
//                 const standardSeats = row.seats?.filter(
//                   (s) => s.type !== "COUPLE",
//                 );
//                 if (!standardSeats || standardSeats.length === 0) return null;
...
//                   (s) => s.type === "COUPLE",
//                 );
//                 if (!coupleSeats || coupleSeats.length === 0) return null;
...
export default function SeatSelection() {
  ...
  const toggleSeat = (seatId: string) => {
    if (!dataSeatMap?.seatLayout) return;
    const next = selectedSeats.includes(seatId)
      ? selectedSeats.filter((s) => s !== seatId)
      : [...selectedSeats, seatId];
    const validation = validateSeatRules(dataSeatMap.seatLayout, next);
    if (!validation.valid) {
      n.error(validation.message);
      return;
    }
  ...
  const showtimeIdNum = showtimeId != null ? Number(showtimeId) : NaN;
  const isValidShowtimeId = !isNaN(showtimeIdNum) && showtimeIdNum > 0;
  const { data: dataSeatMapLayout } = useQuery({
    ...Seat.getSeatMap(showtimeIdNum),
    enabled: isValidShowtimeId,
    refetchOnMount: "always",
    staleTime: 0,
  });
  const dataSeatMap = dataSeatMapLayout?.data;

  useEffect(() => {
    if (!dataSeatMap?.seatLayout) return;
    const map: Record<string, number> = {};
    dataSeatMap.seatLayout.forEach((row) => {
      row.seats?.forEach((seat) => {
        const seatId = seat.code || `${row.rowLabel}${seat.number}`;
        map[seatId] = seat.price ?? 0;
      });
  ...
  const handleContinue = () => {
    ...
    const validation = validateSeatRules(dataSeatMap.seatLayout, selectedSeats);
    if (!validation.valid) {
      n.error(validation.message);
      return;
    }
    ...
    <main className="min-h-screen bg-[#121212] text-slate-200 p-4 md:p-8">
      <div className="container mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-[#1e1e1e] rounded-xl shadow-2xl border border-[#2e2e2e] p-6 md:p-8">
          ...
          <div className="flex flex-col gap-4 items-center overflow-x-auto pb-6">
            <div className="grid gap-3 min-w-[600px]">
              {dataSeatMap?.seatLayout?.map((row) => {
                const standardSeats = row.seats?.filter(
                  (s) => s.type !== "COUPLE",
                );
